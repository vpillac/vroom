...
/**
 * The UnlimitedAsyncAppender lets users log events asynchronously.
 * <p/>
 * <p/>
 * The UnlimitedAsyncAppender will collect the events sent to it and then dispatch them to all the appenders that are
 * attached to it. You can attach multiple appenders to an UnlimitedAsyncAppender.
 * </p>
 * <p/>
 * <p/>
 * The UnlimitedAsyncAppender uses a separate thread to serve the events in its buffer.
 * </p>
 * <p/>
 * <b>Important note:</b> The <code>UnlimitedAsyncAppender</code> can only be script configured using the
...
public class UnlimitedAsyncAppender extends AppenderSkeleton implements AppenderAttachable {
    /**
     ...
     */
    private final List                   buffer              = Collections
    ...
    public void append(final LoggingEvent event) {
        ...
        // See bug 23021
        if ((dispatcherThread == null) || !dispatcherThread.isAlive() || (bufferSize <= 0)) {
            synchronized (appenders) {
        ...
        synchronized (buffer) {
            while (true) {
                ...
                if (blocking && !Thread.interrupted() && Thread.currentThread() != dispatcherThread) {
                    try {
                        buffer.wait();
                        discard = false;
    ...
    /**
     * Close this <code>UnlimitedAsyncAppender</code> by interrupting the dispatcher thread which will process all
     * pending events before exiting.
     */
    @Override
    public void close() {
        /**
         * Set closed flag and notify all threads to check their conditions. Should result in dispatcher terminating.
         */
        synchronized (buffer) {
            closed = true;
            buffer.notifyAll();
        ...
        try {
            ...
            org.apache.log4j.helpers.LogLog.error(
                    "Got an InterruptedException while waiting for the " + "dispatcher to finish.",
                    e);
        }
    ...
    private static final class DiscardSummary {
        ...
        public LoggingEvent createEvent() {
            String msg = MessageFormat.format(
                    "Discarded {0} messages due to full event buffer including: {1}", new Object[] {
    ...
    private static class Dispatcher implements Runnable {
        ...
        /**
         * Parent UnlimitedAsyncAppender.
         */
        private final UnlimitedAsyncAppender parent;
        ...
        public void run() {
            ...
            try {
                //
                // loop until the UnlimitedAsyncAppender is closed.
                //
                while (isActive) {
    ...
    /**
     * Wait until all pending messages are logged
     * 
     * @param timeout
    ...
    public void awaitTermination(int timeout) throws InterruptedException {
        ...
        while (!buffer.isEmpty() && (System.currentTimeMillis() - time) < timeout)
            Thread.sleep(10);
        // close this appender
        close();
        // Await for the dispatcher to finish logging the remaining messages
        while (dispatcher.isRunning() && (System.currentTimeMillis() - time) < timeout)
            Thread.sleep(10);
