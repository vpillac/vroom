...
/**
 * The Class <code>VRPSDDetourRegret</code> is an implementation of the Regret algorithm that considers a subset of
 ...
 * @version 1.0
 */
public class VRPSDDetourRegret extends VRPSDSimpleRegret {

    public static int                                     sSampleSize  = Integer.MAX_VALUE;

    public static double                                  sCostEpsilon = 0.01;

    private final VariableNeighborhoodSearch<VRPScenario> mVNS;

    private final VRPSDConsensus                          mConsensus;

    public VRPSDDetourRegret(ComponentManager<VRPScenario, ?> componentManager) {
    ...
    protected CandidateList selectKLowerInsCost() {
        ...
        ListIterator<CandidateList.Eval> it = (ListIterator<vroom.optimization.online.jmsa.vrp.vrpsd.VRPSDDetourRegret.CandidateList.Eval>) candidateList.bestEvals
                .iterator();
        while (it.hasNext()) {
            if (it.next().eval > avgScenCost * sCostEpsilon) {
                it.remove();
            }
        }
    ...
    protected double evaluateRegret(IActualRequest request, VRPScenario scenario, double currentValue) {
        ...
        double insRelCost = evaluateInsertionCost(depot, route, req) / scenario.getCost();

        double loadRatio = (route.contains(req) ? 1 : 1 + req.getDemand() / route.getLoad());

        return currentValue + insRelCost / loadRatio;
    }
